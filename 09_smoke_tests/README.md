## In this test we're going to create a smoke test. But waht is a smoke test? Basically is a test that verifies that the most important parts of our application works.
...
* Create a new file __./hotel-viewer/cypress/integration/smoke-login.spec.js__
...
* Note here that we're using __context__ allows us to segment our use cases on different contexts. Here we're working on a context that user use valid credentials. The expectation is that navigates to hotels page, so here we're checking that the url contains the expected fragment.

* We can create another one for invalid credentials.
...
* To achieve this goal we're going that our database starts completly clean, that means no hotels.

* Delete entries in __./backend/db.json__, the file has to look this way:
...
```
* Now we can create a new fixture __./hotel-viewer/cypress/fixtures/hotels-server.json__ that will host our hotels.
...
* Now we can create new smoke test related with hotels. Create __./hotel-viewer/cypress/integration/smoke-hotels.spec.js__. Lets create the test declaration that we want for this hotels smoke tests.
